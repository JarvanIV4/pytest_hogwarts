...
#定义测试套件,在定义好的路径中自动发现用例，自动添加crm开头的脚本
# testcase_dir = r"D:\python_work\auto_test\ui_test\src\testcase\\"
suit = unittest.defaultTestLoader.discover(config.testcase_path,"crm_*.py")

#定义执行器
# runner = ui_test.TextTestRunner()
# runner.run(suit)
#定义报告生成路径
report_path = r"D:\python_work\auto_test\report\\"
#定义测试报告的文件名
now = time.strftime("%Y-%m-%d_%H-%M-%S",time.localtime())
report_name = config.report_path+"AutoTest"+now+".html"
