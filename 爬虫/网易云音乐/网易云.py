...
def get_music(ID, path='D:\\网易云音乐-歌单\\'):
    """
    下载单个歌曲
    :param ID: 歌曲的id信息
    :param path: 歌曲的下载路径
...
def get_music_list(playlist_id):
    ...
    response = s.get(base_url + str(playlist_id), headers=headers).content
    ss = BeautifulSoup(response, 'html.parser')  # 使用bs4解析
    main = ss.find('ul', {'class': 'f-hide'})
    ls = main.find_all('a')     # ls 是迭代器 存放所有歌的标签
    global title
    ...
    for music in ls:
        ...
        print('Name: {:<10} ID: {:<30}'.format(MID, Mname))
        Music_dic[Mname] = MID
...
def get_playlist_id(playlist_id):
    ...
    ls = re.findall(pattern, str(playlist_id))
    if ls:
...
def main(playlist_id, path=path):
    '''
    主函数 执行下载
    :return: None
    '''
    while True:
        ID, flag = get_playlist_id(playlist_id)
        if flag:
            listJson = get_music_list(ID)
            # 创建文件夹
            ...
            for name in dic.keys():
                ...
                get_music(dic[name], path + title + '/' + name + '.mp3')
            break
        else:
...
if __name__ == '__main__':
    main(3212088554)  # 3205283801/3212088554/3197355233/2790839513
